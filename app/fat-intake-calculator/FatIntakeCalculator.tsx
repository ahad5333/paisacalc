...
export function FatIntakeCalculator({ content }: { content: ReactNode }) {
  ...
  const result = calculateFat({ sex, age, heightCm, weightKg, activityLevel, goal });
  const { dailyCalorieTarget, fatLowG, fatHighG, saturatedFatCapG } = result.value;
  ...
  return (
    <CalculatorPage
      ...
      description="Recommended daily fat range in grams, plus a saturated fat cap, from the US Dietary Guidelines applied to your calorie target."
      ...
      result={
        <ResultDisplay
          value={`${fatLowG}–${fatHighG}g`}
          caption={`20–35% of your ${dailyCalorieTarget} kcal target — keep saturated fat under ${saturatedFatCapG}g`}
        />
      }
