...
import { calculateVolume, type SolidShape } from "@/lib/calc/volume";
...
export function VolumeCalculator({ content }: { content: ReactNode }) {
  const [shape, setShape] = useState<SolidShape>("cube");
  const [a, setA] = useState(3);
  const [b, setB] = useState(4);
  const [height, setHeight] = useState(5);

  const result = calculateVolume({ shape, a, b, height });
  ...
  return (
    <CalculatorPage
      ...
      description="Volume of a cube, box, sphere, cylinder, or cone."
      ...
      result={<ResultDisplay value={`${volume}`} caption={`Volume (cubic units) — ${shape}`} />}
